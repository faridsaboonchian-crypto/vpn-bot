...
import jdatetime  # 📆 کتابخانه تبدیل تاریخ شمسی
import os  # 🔑 خواندن متغیرهای محیطی سیستم‌عامل
from dotenv import load_dotenv  # 🔑 بارگذاری فایل .env
import sqlite3  # 🗄️ کتابخانه پایگاه داده SQLite
import threading  # 🧵 کتابخانه چندنخی برای اجرای پس‌زمینه

# بارگذاری متغیرهای امنیتی از فایل محرمانه .env در سرور
...
# متغیر شناسه اینباند پویا از فایل .env (مقدار پیش‌فرض 19)
INBOUND_ID = int(os.getenv("INBOUND_ID", 19))
# ====================================================================
...
USER_SELECTED_PLAN = {}
USER_LAST_INTERACTION = {}  # 🔑 دیکشنری زمان آخرین فعالیت کاربران برای سیستم ضد اسپم

USER_APPLIED_DISCOUNT = {}  # 🔑 ذخیره موقت کد تخفیف اعمال شده برای هر کاربر در این خرید

# کدهای تخفیف فعال و درصد تخفیف آن‌ها (قابل توسعه و تغییر به دلخواه شما)
DISCOUNT_CODES = {
    "MOHARAM20": 20,  # ۲۰ درصد تخفیف
...
# تابع مقداردهی اولیه دیتابیس و مهاجرت خودکار داده‌های قدیمی
def init_db():
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    
    # ابتدا جدول را به صورت ساختار ساده پایه می‌سازیم
    cursor.execute('''
    ...
    # بررسی و اضافه کردن ستون‌های مورد نیاز در صورت عدم وجود
    ...
    # انتقال خودکار اطلاعات کاربران قبلی از فایل اصلی یا فایل پشتیبان قدیمی
    ...
    if os.path.exists(json_path):
        ...
        # بررسی اینکه آیا دیتابیس اس‌کیوال خالی است؛ اگر خالی بود از بکاپ مهاجرت کن
    ...
    if source_path:
        try:
            ...
            # اگر فایل اصلی بود، آن را به بکاپ تغییر نام بده
...
# ایجاد جدول پایش کدهای تخفیف مصرف شده هر کاربر
    cursor.execute('''
...
# تبدیل قیمت فارسی کامادار به عدد صحیح انگلیسی
def parse_price(price_str):
    cleaned = price_str.replace(",", "").replace("،", "")
    en_str = fa_to_en_num(cleaned)
    return int(en_str)

# تبدیل عدد صحیح انگلیسی به فرمت قیمت فارسی کامادار
def format_price(price_int):
    formatted_en = f"{price_int:,}"
    return en_to_fa_num(formatted_en)

# بررسی اینکه آیا کاربر قبلاً از این کد استفاده کرده است یا خیر
...
# ثبت نهایی استفاده از کد تخفیف برای کاربر در دیتابیس
...
# تابع یکپارچه نمایش اطلاعات کارت پرداخت با قیمت نهایی
def render_payment_card(message, chat_id, plan_key):
    plan = PLANS[plan_key]
    price_str = plan['price']
    
    # بررسی اعمال کد تخفیف روی این خرید
...
# تابع بررسی وضعیت یک کاربر در دیتابیس
...
# تابع ثبت یا بروزرسانی نام و یوزرنیم کاربر جدید/فعلی در دیتابیس (اصلاح نهایی با تصحیح تعداد پارامترهای درج)
def add_or_update_user(chat_id, first_name, username, referred_by=None):
    ...
    try:
        ...
        if exists:
            ...
            # همگام‌سازی و اصلاح ساختاری تعداد پارامترها و مقادیر ورودی در دیتابیس
            cursor.execute("""
...
# تابع ثبت دریافت پاداش دعوت از دوستان در دیتابیس
# تابع ثبت دریافت پاداش دعوت از دوستان در دیتابیس (مجهز به کنترل خطای سخت‌گیرانه)
def set_received_referral_reward(chat_id):
    ...
    try:
        ...
        raise e  # خطا را به بالا گزارش بده تا فرآیند هدیه متوقف شود
    finally:
        conn.close()

# تابع ثبت دریافت اکانت تست رایگان در دیتابیس
...
# تابع بروزرسانی تعداد دعوت‌های یک کاربر
...
# تابع استعلام تعداد کل اعضای ربات برای پنل ادمین
def get_total_users():
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    cursor.execute("SELECT COUNT(*) FROM users")
...
# تابع استعلام اعضای اخیر همراه با نام و یوزرنیم ذخیره شده (بدون نیاز به درخواست از تلگرام)
...
# تابع واکشی لیست آیدی تمام کاربران جهت ارسال پیام همگانی
...
# پردازه پس‌زمینه جهت استعلام تدریجی و ایمن مشخصات کاربران قدیمی بدون ریت‌لیمیت تلگرام
def sync_profiles_background():
    ...
    try:
        ...
        for chat_id, referred_by in rows:
            try:
                # استعلام مشخصات کاربر با وقفه ایمن جهت عبور از فیلتر نرخ تلگرام
                ...
                # ثبت وضعیت آفلاین در دیتابیس برای جلوگیری از تلاش مجدد و فریز شدن
            ...
            # وقفه ایمن ۱.۵ ثانیه‌ای بین هر استعلام برای جلوگیری از ریت‌لیمیت
            time.sleep(1.5)
...
# تابع مدیریت ضد اسپم و ریت‌لیمیت برای کاربران
...
# تولید رشته تصادفی برای شناسه سابسکریپشن
def generate_sub_id():
    return ''.join(random.choices(string.ascii_lowercase + string.digits, k=16))

# تابع تبدیل بایت به گیگابایت برای مانیتورینگ
def bytes_to_gb(b):
    return round(b / (1024 * 1024 * 1024), 2)

# تابع فرمت‌دهی زمان انقضا برای مانیتورینگ فارسی
def format_date_shamsi_with_countdown(expiry_time_ms):
    ...
    # محاسبه دقیق روزها و ساعت‌های باقی‌مانده
...
# تابع تست اتصال به پنل در استارت‌آپ
def test_api():
...
# تابع ساخت کاربر (صدور کانفیگ و UUID جدید با نام کاربری یکتا)
# متغیرهای جدید CDN از فایل .env
CLEAN_IP = os.getenv("CLEAN_IP", "104.18.23.10")
    ...
    try:
        ...
        payload = {
            "client": {
                "id": client_uuid,
                "email": email,
                "flow": "",  # 🔴 برای WebSocket باید خالی باشد
                "limitIp": 2,
        ...
        if response.status_code == 200:
            ...
            if res_data.get('success') == True:
                # 🔴 انتخاب تصادفی IP از لیست برای توزیع بار
                clean_ips = [ip.strip() for ip in CLEAN_IP.split(',')]
                selected_ip = random.choice(clean_ips)
                
                # 🔴 ساخت لینک VLESS با تنظیمات Cloudflare
                vless_link = (
                    f"vless://{client_uuid}@{selected_ip}:80"
                ...
                logging.info(f"لینک ساخته شد: {vless_link}")
                return vless_link
...
# تابع مانیتورینگ زنده (پایش اکانت‌های پولی و هدیه)
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get("success") == True:
                # تعریف الگوهای مختلف ایمیل مربوط به این کاربر
                ...
                # سرچ کلاینت‌های اینباند معرفی شده در .env
                for inbound in data.get("obj", []):
                    if inbound.get("id") == INBOUND_ID:
                        for client in inbound.get("clientStats", []):
                            email = client.get("email", "")
                            # بررسی اینکه آیا ایمیل کلاینت متعلق به این کاربر (پولی، هدیه یا تست) است
                ...
                for index, client in enumerate(found_clients, 1):
                    ...
                    # تفکیک نوع اکانت بر اساس پیشوند ایمیل کلاینت
...
# ==================== [ بخش دکمه‌ها و مدیریت ربات ] ====================
# عملکرد دکمه ورود و شروع کار با ربات تلگرام
...
def send_welcome(message):
    ...
    # استخراج نام و یوزرنیم تلگرام کاربر جدید (دعوت‌شونده)
    ...
    # ثبت کاربر جدید (دعوت‌شونده) در دیتابیس
    if not user_data:
        is_new = True
        ...
        # اگر کاربر قدیمی است، نام و یوزرنیم جدید او را در جدول آپدیت کن
        add_or_update_user(chat_id, first_name, username, user_data.get("referred_by"))

    # منطق پاداش دعوت برای دعوت‌کننده (میزبان)
    if is_new and referrer_id:
        # ۱. ابتدا تعداد دعوت‌های دعوت‌کننده را ۱ واحد در دیتابیس افزایش می‌دهیم
        update_invite_count(referrer_id, 1)
        
        # ۲. واکشی مستقیم و تازه وضعیت دعوت‌کننده (میزبان) از دیتابیس بلافاصله پس از آپدیت
        ...
        if inviter_data:
            ...
            # [لاگ ۱ دیباگ]: چاپ وضعیت دقیق دعوت‌کننده قبل از ارزیابی شرط هدیه
            ...
            # ۳. اطلاع‌رسانی عضویت کاربر جدید به دعوت‌کننده بر اساس وضعیت پاداش او
            try:
            ...
            # ۴. گیت شرطی بسیار سخت‌گیرانه قبل از صدور هرگونه پاداش
            if current_invites >= 3 and has_received_reward == 0:
                # [لاگ ۲ دیباگ]: تایید برقراری شرط هدیه دعوت
                ...
                try:
                    # ۵. اولویت‌بندی مطلق دیتابیس بر وب‌سرویس (تغییر وضعیت فیلد به ۱ قبل از صدور لینک)
                    set_received_referral_reward(referrer_id)
                    
                    # [لاگ ۳ دیباگ]: تایید قطعی اجرای تراکنش SQL روی دیتابیس
                    ...
                    # ۶. حالا اقدام به ساخت اکانت هدیه ۲ گیگی از طریق API پنل سنایی می‌کنیم
                    suffix = ''.join(random.choices(string.ascii_lowercase + string.digits, k=4))
                    free_username = f"free_{referrer_id}_{suffix}"
                    
                    # اطلاع به مدیر در کانال ادمین
                    ...
                    if vless_link:
                        gift_text = (
                            ...
                            "📌 کادر بالا را لمس (کپی) کنید و در برنامه v2rayNG پیست فرمایید. از اینترنت آزاد لذت ببرید! 🌹"
                        )
                        try:
                    ...
                    # در صورت بروز خطای دیتابیس، این بلاک مانع از پیشروی شده و API کلاینت سنایی صدا زده نمی‌شود
                    logging.error(f"خطای جدی دیتابیس. عملیات صدور لایسنس برای جلوگیری از تداخل لغو شد: {db_error}")
            else:
                # [لاگ ۲ دیباگ - منفی]: عدم احراز شرایط پاداش
...
# هندلر دکمه Reply دعوت از دوستان با منوی جدید شیشه‌ای
...
# هندلر Callback Query دکمه‌های شیشه‌ای منوی هدیه و تست
...
def handle_free_features(call):
    ...
    if call.data == "get_test_config":
        ...
        # ساخت پسوند تصادفی برای ایمیل کلاینت
        suffix = ''.join(random.choices(string.ascii_lowercase + string.digits, k=4))
        test_username = f"free_test_{chat_id}_{suffix}"
        
        # ایجاد اکانت تست با حجم ۱ گیگابایت و ۱ روز انقضا
        vless_link = create_vless_link(test_username, limit_gb=1, expiry_days=1)
        
        if vless_link:
            # ثبت دریافت اکانت تست در دیتابیس
        ...
        # نمایش وضعیت پیشرفت شخصی‌سازی شده بر اساس وضعیت دریافت پاداش یکبار مصرف
        if has_received_reward == 1:
        ...
        forward_msg = (
            "🚀 **اینترنت پرسرعت، امن و بدون قطعی Reality آلمان!**\n\n"
...
# عملکرد دکمه وضعیت اشتراک (بدون هیچ دکمه یا مارک‌آپ تمدید)
...
def ask_discount_prompt(call):
    ...
    # ریست کردن تخفیف‌های احتمالی قبلی برای این خرید جدید
...
# ادامه خرید مستقیم بدون اعمال تخفیف
...
# درخواست تایپ کد تخفیف از کاربر
...
def ask_for_discount_code(call):
    ...
    # برای سازگاری کامل با تمامی نسخه‌های قدیمی و جدید کتابخانه telebot
...
# پردازش و اعتبارسنجی کد تخفیف ارسال شده
def process_discount_code(message, plan_key):
    chat_id = message.chat.id
    raw_text = message.text.strip() if message.text else ""
    
    # پاک‌سازی همه‌جانبه: تبدیل اعداد فارسی، حذف اسلش (/)، بک‌اسلش (\) و فضاهای خالی چپ و راست
    code = fa_to_en_num(raw_text).replace("/", "").replace("\\", "").strip().upper()
    
    # 📌 سیستم دیباگ زنده در ترمینال سرور (ترمیوس)
    # به محض اینکه کاربر کد را بفرستد، این لاگ‌ها در ترمینال شما چاپ می‌شوند تا بررسی کنید سیستم چه چیزی دریافت کرده است.
    ...
    # بررسی صحت وجود کد در کدهای مجاز تعریف شده در بالای فایل
    ...
    # بررسی سابقه استفاده کاربر از این کد مشخص در دیتابیس SQLite
    ...
    # ذخیره موقت کد تخفیف برای کاربر و رندر نهایی کارت واریز با قیمت کاهش یافته
...
def handle_receipt(message):
    ...
    # محاسبه قیمت نهایی فاکتور با احتساب تخفیف اعمال شده
    plan = PLANS[plan_key]
    price_str = plan['price']
    ...
    # ارسال کد تخفیف اعمال شده به عنوان پارامتر آخر در دیتای دکمه تایید ادمین
    markup = types.InlineKeyboardMarkup()
    markup.add(
...
def handle_admin_action(call):
    ...
    if action == "approve":
        ...
        # ارسال پویا و دقیق تعداد روزهای پلن به متد ساخت اکانت سنایی
        ...
        if vless_link:
            # ثبت دائم استفاده از کد تخفیف در دیتابیس SQLite پس از تایید نهایی فیش توسط ادمین
            if discount_code != "none":
                mark_discount_used(target_user_id, discount_code)
                
            success_text = f"پرداخت شما تایید شد! 🎉\n\n🚀 <b>لینک اشتراک جدید شما:</b>\n\n<code>{vless_link}</code>"
            bot.send_message(target_user_id, success_text, parse_mode="HTML")
            bot.send_message(ADMIN_CHAT_ID, "لینک با موفقیت صادر شد. ✅")
        else:
...
# 📊 آمار رسمی اعضا مخصوص پنل ادمین (مجهز به سیستم خودترمیم مشخصات کاربران قدیمی)
    ...
    try:
        ...
        if not items:
            ...
            for chat_id, data in items:
                ...
                # مکانیزم لودینگ تنبل (Self-Healing Cache):
                # اگر کاربر قدیمی است و نام او در دیتابیس خالی است، یک‌بار از تلگرام بپرس و برای همیشه ذخیره کن
                if not first_name:
                    try:
                        chat_obj = bot.get_chat(int(chat_id))
                        first_name = chat_obj.first_name or "کاربر ناشناس"
                        username = chat_obj.username
                        # ذخیره مشخصات کاربر در دیتابیس برای دفعات بعدی
                        add_or_update_user(chat_id, first_name, username, ref_by)
                    except Exception:
                        # در صورتی که کاربر ربات را بلاک کرده باشد یا اکانت دی‌اکتیو شده باشد
                        first_name = "کاربر قدیمی (آفلاین)"
                        username = None
                
                # فرار از کاراکترهای مخل ساختار HTML در تلگرام
                if first_name:
        ...
        # ارسال بدون پارس مد جهت تضمین تحویل تحت هر شرایطی
        bot.send_message(ADMIN_CHAT_ID, f"❌ خطای اجرای دستور:\n{str(e)}")

# 📢 دستور ادمین برای ارسال پیام همگانی
...
# ایجاد دیتابیس اس‌کیوال و مهاجرت اطلاعات کاربران
init_db()

# اجرای ایمن پردازه پس‌زمینه بروزرسانی مشخصات در ترد مستقل بدون فریز کردن ربات
threading.Thread(target=sync_profiles_background, daemon=True).start()

# تست اتصال وب‌سرویس در بدو ورود ربات
test_api()
